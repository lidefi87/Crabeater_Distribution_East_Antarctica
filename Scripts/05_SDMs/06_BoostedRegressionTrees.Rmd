...
Based on AUC values, BRTs are the second best performing SDM algorithm after Random Forest (AUC test = 0.948).  
...
TSS is more than double than the one obtained in MaxEnt (0.41), but slightly lower than Random Forest (0.98). This result suggests this model has an excellent performance.  
...
In total, six variables were removed: bottom velocity along longitudes (`vel_lon_bottom_msec`) and latitudes (`vel_lat_bottom_msec`), the slope of the seafloor (`bottom_slope_deg`), `month`, long-term presence of sea ice (`lt_pack_ice`), and depth (`depth_m`). Although model performance was not affected by the removal of these variables, the resulting map showed that the probability of crabeaters being present in the area of interest was mostly zero. However, when all variables were included, results made more sense based on what we know about crabeaters. So, we will produce a final report for the model that includes all variables and predict crabeater distribution based on the full model.  
