...
We have identified three variables that we can remove without affecting the predictive performance of the model: `month`, the slope of the seafloor (`bottom_slope_deg`) and distance to sea ice edge (`dist_ice_edge_km`). However, we still have some variables that were highly correlated: distance to continental shelf (`dist_shelf_km`) and depth (`depth_m`), and sea surface temperature (`SST_degC`) and `SIC`.   
...
First, we will start by removing the three variables identified as not significantly contributing to the model `month` and the slope of the seafloor (`bottom_slope_deg`), as well as `depth_m`. Then, we will remove long-term pack ice presence (`depth_m`) and calculate AUC values and ROC curves.  
  
```{r}
#Dataset for training and testing - excluding low contribution variables and depth
model_data_nodepth <- mod_match_obs %>% 
  select(!c(year, month, bottom_slope_deg, dist_ice_edge_km, depth_m)) %>% 
  sdm_format() %>% 
  trainValTest(test = 0.25, only_presence = T, seed = 42)

#Train model
simple_model_nodepth <- train(method = "Maxent", fc = "lqpht", reg = 0.5, iter = 1000,
...
For reference, the AUC from the original model we are testing against was `0.661` (against testing dataset). By removing `depth`, the AUC increased slightly to `0.701` (testing data). Now we will remove `dist_shelf_km` and compare its performance.   
  
```{r}
#Dataset for training and testing - excluding low contribution variables and depth
model_data_nodistshelf <- mod_match_obs %>% 
  select(!c(year, month, bottom_slope_deg, dist_ice_edge_km, dist_shelf_km)) %>% 
  sdm_format() %>% 
  trainValTest(test = 0.25, only_presence = T, seed = 42)

#Train model
simple_model_nodistshelf <- train(method = "Maxent", fc = "lqpht", reg = 0.5, iter = 1000,
                               data = model_data_nodistshelf[[1]])

#Plot ROC curves
plotROC(simple_model_nodistshelf, test = model_data_nodistshelf[[2]])
```
  
The AUC is the same as when removing `depth`, we will drop `dist_ice_edge_km` because its importance was lower than `depth`. Now will test the effect of removing `SST` and `SIC`.  
  
```{r}
#Dataset for training and testing - excluding low contribution variables, depth and SST
model_data_noSST <- mod_match_obs %>% 
  select(!c(year, month, bottom_slope_deg, dist_ice_edge_km, dist_shelf_km, SST_degC)) %>% 
  sdm_format() %>% 
  trainValTest(test = 0.25, only_presence = T, seed = 42)

#Train model
simple_model_noSST <- train(method = "Maxent", fc = "lqpht", reg = 0.5, iter = 1000,
...
Removing `SST` resulting in a decline in AUC from 0.662 to 0.629. Finally, we will test the effect of removing `SIC`.  
  
```{r}
#Dataset for training and testing - excluding low contribution variables, depth and SIC
model_data_noSIC <- mod_match_obs %>% 
  select(!c(year, month, bottom_slope_deg, dist_ice_edge_km, dist_shelf_km, SIC)) %>% 
  sdm_format() %>% 
  trainValTest(test = 0.25, only_presence = T, seed = 42)

#Train model
simple_model_noSIC <- train(method = "Maxent", fc = "lqpht", reg = 0.5, iter = 1000,
...
The removal of `SIC` had a lower impact on performance. We will check our dataset for multicollinearity once more.  
...
#Best model based on test AUC and smallest AUC difference between train and test
best_max_mod <- gs_mod@models[[105]]
...
modelReport(best_max_mod, type = "cloglog", folder = file.path(out_folder, "reduced_Maxent_model/"),
            test = model_data_noSIC[[2]], response_curves = T, only_presence = T, jk = T)
```
