...
```{r}
# Most complex model
full_model <- presence ~ month + s(year) + s(bottom_slope_deg) + s(dist_coast_km) + s(depth_m) + s(SST_degC, by = month) + s(lt_pack_ice, by = month) + s(dist_ice_edge_km, by = month)
```
...
Removing variables with effects reduced to near zero (`edf` < 0.001) or that are non-significant: `year`, `bottom_slope_deg` (seafloor slope), and `dist_coast_km` (distance to coastline in km).
