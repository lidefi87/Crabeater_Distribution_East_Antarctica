...
mean_model <- read_csv("Cleaned_Data/All_values_ACCESS-OM2-01_env_vars.csv")
...
res_back %>% 
  write_csv("pred_mean_env_best_gam_model.csv")
...
library(mgcv)
library(splines)
mod_gam <- gam(formula = presence ~ ns(bottom_slope_deg, df = 2) + dist_shelf_km + depth_m + ns(SIC, df = 5) + ns(bottom_temp_degC, df = 5) + ns(SSS_PSU, df = 5) + vel_lat_surf_msec + ns(vel_lon_bottom_msec, df = 2) + ns(lt_pack_ice, df = 5) + ns(dist_ice_edge_km, df = 5), 
...
summary(cv_mod_no_month)


#Checking predictions
res <- test %>%  
  mutate(pred = predict(mod_gam_no_month, test, type = "response"))
...
pred_mean_model <- mean_model %>%  
  mutate(pred = as.vector(predict(mod_gam_no_month, 
                        mean_model %>% select(bottom_slope_deg:dist_ice_edge_km),
                        type = "response")))
