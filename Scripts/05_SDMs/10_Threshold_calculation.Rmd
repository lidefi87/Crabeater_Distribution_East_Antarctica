---
title: "Calculating threshold for individual SDM algorithms"
...
We calculated max SSS threshold using an adapted form of the `thresholds` function from the `SDMTune` package to find the probability above which the crabeaters are considered to occur. We will use these threshold to create binary maps of presence that we will use as a measure of uncertainty. Any probabilities above or equal to the threshold will be classified as presence (value of 1), while any values below the threshold were considered absence (value of 0).  
